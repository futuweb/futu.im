...
![起始文件](/images/2018-04-01-how-to-hand-code-svg/starterhtml.png)

## `x` 和 `y` 值概述

在网站的二维平面上，用 `x` 代表水平轴，`y` 代表竖直轴。每个坐标轴上的位置都用数字表示。使用递增的 `x` 值来让物体左移，使用递减的 `x` 值来让物体右移。同样地，使用递增的 `y` 值来让物体下移，使用递减的 `y` 值来让物体上移。
...
让我们创建我们的图标的第一条线，即顶部的那条。我们打算让线长 `45px`，然而我们使用的 `5px` 粗细的笔画会在我们的线条外增加一些额外的像素。因此我们需要将线条向右下角偏移 `3px` 以确保所有因笔画产生的额外像素都不会被剪掉。
...
现在我们所要做的就是在顶部添加一条水平线，还有在顶部偏左处加上一条竖线，就像你在本节开始时所看到图像中的那样。我们将使用与之前绘制线段相同的过程，完整的 `浏览器` 图标代码看起来应该是这样的：
...
这样的命令有很多，但是为了仅给出一个使用 `<path>` 的介绍，我们将坚持用一些在手动编码时可以实实在用到的东西。
...
如果你想要把我们目前创建的图标设置为居中，这将非常有用。我们将把剩下的三个图标转换成模板，然后调整它们在竖向上充满 `100px` 高的空间，并在这个空间中水平居中。
