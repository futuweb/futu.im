...
cover: /images/2018-09-04-create-snapshot/cover.jpg
...
后来，我们想能不能直接有前端生成这张分享快照呢？前端生成图片，无需经过网络传输等操作，可减少服务的压力，且可以每次都动态生成图片，可以使分享图片展示一些随用户操作而不断变化的数据。之后发现已经存在一些`html`生成`canvas`或者`img`的库，所以我们就试了一下，看能不能直接由前端生成分享快照。
